...
> 北京交通大学校园网自动登录工具 — 连接 WiFi 后自动认证，断网自动重连，基于 PyWebView 的现代化桌面应用。

## 功能

- **自动登录** — 检测到校园网 WiFi 后自动完成 Portal 认证，断网自动重连
- **状态监控** — 实时显示 WiFi 连接状态、SSID 和外网连通性
- **GUI 界面** — PyWebView + HTML/CSS 无边框窗口，支持拖拽移动和最小化
- **开机自启** — 通过 Windows 任务计划程序实现，GUI 内一键开关
...
**Q: 如何卸载？**  
A: 在 GUI 中关闭"开机自启"开关，然后删除程序目录即可。
